...
def sign_up():
    ...
    if len != 4:
        flash("L.E.N must be equal to four digit.", category="error")
    elif len(email) <= 10:
        ...
        new_user = User(email=email, name=name, len=len, password=generate_password_hash(password1, method="sha256"))
